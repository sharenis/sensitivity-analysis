...
def run_simulation(Sh, Ih, Rh, Sv, Iv, A, Bh, Mh, u1, a, II, Bv, u2min, Ohm2, Ohm21, Ohm22, Ohm23, Ohm24, Mv, t_awal, t_akhir, h):
    # Parameter waktu
    ...
    # Inisialisasi vector J dan F awal
...
# Streamlit UI untuk parameter visualisasi
st.title('Sensitivity Analysis Visualization')

# Parameter selection for each variable
selected_parameters = {var: st.multiselect(f'Select parameters for {var}', parameter_names, default=parameter_names)
                       for var in variable_names}

# Jalankan simulasi
results = run_simulation(
    Sh, Ih, Rh, Sv, Iv, A, Bh, Mh, u1, a, II, Bv, u2min, 
    Ohm2, Ohm21, Ohm22, Ohm23, Ohm24, Mv, t_awal, t_akhir, h
)
...
if st.button('Run Sensitivity Analysis'):
    for i, variable_name in enumerate(variable_names):
        ...
        selected_indices = [parameter_names.index(param) for param in selected_parameters[variable_name]]
        
        # Plot the selected parameters for the current variable
        for index in selected_indices:
            plt.plot(
                range(results.shape[0]),  # Time steps
                results[:, i, index],  # Data for the selected parameter
                label=f'{parameter_names[index]}',
                linewidth=2
            )
        
        # Add title and labels
        plt.title(f'Sensitivity Analysis of {variable_name}', fontsize=16, fontweight='bold')
        plt.xlabel('Time Steps', fontsize=12)
        plt.ylabel('Sensitivity', fontsize=12)
        plt.legend(title='Parameters', loc='upper right', fontsize=10)
